...
describe('SnapViewer', () => {
  ...
  it('loads and displays the image from filePath query param', async () => {
    ...
    expect(window.snap.viewer.readImage).toHaveBeenCalledWith(TEST_FILE_PATH);
  });
  ...
  it('does not load image when filePath is missing', () => {
    ...
    expect(window.snap.viewer.readImage).not.toHaveBeenCalled();
  ...
  it('copies image on Cmd+C', async () => {
    ...
    expect(window.snap.viewer.copy).toHaveBeenCalledWith(TEST_FILE_PATH);
  });
  ...
  it('does not copy without meta key', async () => {
    ...
    expect(window.snap.viewer.copy).not.toHaveBeenCalled();
  });
  ...
  it('toggles shadow on Cmd+P', async () => {
    ...
    expect(window.snap.viewer.toggleShadow).toHaveBeenCalled();
  });
  ...
  it('closes on double click', async () => {
    ...
    expect(window.snap.viewer.close).toHaveBeenCalled();
  });
  ...
  it('opens context menu popup on right click', async () => {
    ...
    expect(window.snap.viewer.openMenu).toHaveBeenCalled();
  });
  ...
  it('adjusts opacity on wheel scroll down (more transparent)', async () => {
    ...
    expect(window.snap.viewer.setOpacity).toHaveBeenCalledWith(0.95);
  });
  ...
  it('adjusts opacity on wheel scroll up (more opaque)', async () => {
    ...
    expect(window.snap.viewer.setOpacity).toHaveBeenLastCalledWith(1);
  });
  ...
  it('clamps opacity to minimum 0.05', async () => {
    ...
    const calls = vi.mocked(window.snap.viewer.setOpacity).mock.calls;
  ...
  it('moves window on pointer drag', async () => {
    ...
    expect(window.snap.viewer.move).toHaveBeenCalledWith(10, 15);
  });
  ...
  it('does not move on right-button drag', async () => {
    ...
    expect(window.snap.viewer.move).not.toHaveBeenCalled();
  });
});
